...
@section('page_title', 'Reimbursement')
...
<div class="relative overflow-x-auto pb-10">
    ...
    <div class="flex items-center justify-between mb-3">
        <div class="text-sm">
            ...
            <a href="{{ route('reimbursements.index') }}" class="text-sm text-blue-400">
                [ Clear Search ]
            </a>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div id="approve-modal-{{ $reimbursement->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                        data-modal-hide="approve-modal-{{ $reimbursement->id }}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <form action="{{ route('reimbursements.approve', $reimbursement->id) }}"
                                            method="POST">
                                            @method('PATCH')
                                            @csrf
                                            ...
                                            <div class="flex items-center gap-1 justify-center">
                                                ...
                                                <button data-modal-hide="approve-modal-{{ $reimbursement->id }}"
                                                    type="button"
                    ...
                    <td class="px-6 py-4">
                        ...
                        <button data-modal-target="note-modal-{{ $reimbursement->id }}"
                            data-modal-toggle="note-modal-{{ $reimbursement->id }}"
                        ...
                        @else
                        <svg data-tooltip-target="forbidden-note-{{ $reimbursement->id }}"
                        ...
                        <div id="forbidden-note-{{ $reimbursement->id }}" role="tooltip"
                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                            Tidak dapat menambahkan keterangan karena belum disetujui
                            <div class="tooltip-arrow" data-popper-arrow></div>
                        </div>
                        @endif
                        <div id="note-modal-{{ $reimbursement->id }}" tabindex="-1" aria-hidden="true"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div
                                        ...
                                        <button type="button"
                                            ...
                                            data-modal-hide="note-modal-{{ $reimbursement->id }}">
                                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                                fill="none" viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round"
                                                    stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="p-4 md:p-5">
                                        <form class="space-y-4"
                                            action="{{ route('reimbursements.note', $reimbursement->id) }}"
                                            method="POST">
                                            @method('PATCH')
                                            @csrf
                                            <div>
                                                <textarea id="note" rows="4" name="note"
                                                    ...
                                                    required
                                                    placeholder="Tulis keterangan di sini...">{{ $reimbursement->note }}</textarea>
                                            </div>
                                            <button type="submit"
                    ...
                    <td class="px-6 py-4">
                        <button data-popover-target="action-{{ $reimbursement->id }}" data-popover-placement="left"
                            type="button"
                        ...
                        <div data-popover id="action-{{ $reimbursement->id }}" role="tooltip"
    ...
    <nav class="flex items-center justify-between mt-4">
        <div class="text-gray-500">
            Showing {{ $reimbursements->firstItem() }} to {{ $reimbursements->lastItem() }} of {{
            $reimbursements->total() }}
            results
        </div>
        <div class="flex -space-x-1">
            @if ($reimbursements->currentPage() > 1)
            <a href="{{ $reimbursements->previousPageUrl() }}"
                class="px-4 py-2 rounded-md hover:bg-gray-100">Previous</a>
            @endif
            @php
            $totalPages = $reimbursements->lastPage() >= 10 ? 10 : $reimbursements->lastPage();
            @endphp
            @for ($i = 1; $i <= $totalPages; $i++) @if ($i==$reimbursements->currentPage())
                <span class="px-4 py-2 text-gray-700 bg-gray-100 rounded-md">{{ $i }}</span>
                @else
                <a href="{{ $reimbursements->appends(['q' => request()->query('q'), 'other' => request()->query('other')])->url($i) }}"
                    class="px-4 py-2 rounded-md hover:bg-gray-100">{{ $i }}</a>
                @endif
                @endfor
                @if ($reimbursements->currentPage() < $reimbursements->lastPage())
                    <a href="{{ $reimbursements->nextPageUrl() }}"
                        class="px-4 py-2 rounded-md hover:bg-gray-100">Next</a>
                    @endif
        </div>
    </nav>
    @endif
