...
@section('page_title', 'Tambah Laporan Pemakaian CA')

@section('admin_content')
<form action="{{ route('ca-usages.store') }}" method="POST">
    <div class="relative overflow-x-auto px-2">
        <div class="flex items-center gap-1 justify-end mt-3">
            <a href="{{ route('ca-usages.index') }}" class="text-blue-600 px-4">Cancel</a>
        ...
        @csrf
        @include('ca-usage.partials.form')
    </div>
...
<div class="relative overflow-x-auto px-2 mt-5">
    ...
    @enderror
    <div id="item-form">
        @include('ca-usage-item.partials.form')
    </div>
    ...
    @include('ca-usage-item.partials.table', ['items' => []])
</div>
@endsection

@section('scripts')
@vite('resources/js/manage-ca-usage-item.js')
