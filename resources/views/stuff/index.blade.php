...
@section('page_title', 'Pengajuan Barang & ATK')
        ...
        @if (request()->query('q'))
            <div class="flex items-center justify-between mb-3">
                <div class="text-sm">
                    ...
                    <a href="{{ route('stuffs.index') }}" class="text-sm text-blue-400">
                        [ Clear Search ]
                    </a>
                </div>
            </div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                @foreach ($stuffs as $stuff)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            {{ Carbon\Carbon::parse($stuff->date)->format('d/m/Y') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $stuff->user->name }}
                        </td>
                        <td class="px-6 py-4">
                            @if ($stuff->is_pending && auth()->user()->role_id ==
                            Database\Seeders\RoleSeeder::ADMIN_ID)
                                <button data-modal-target="approve-modal-{{ $stuff->id }}"
                                        data-modal-toggle="approve-modal-{{ $stuff->id }}" type="button"
                                    ...
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <form action="{{ route('stuffs.approve', $stuff->id) }}" method="POST">
                                                    @method('PATCH')
                                                    @csrf
                                                    ...
                                                    <div class="flex items-center gap-1 justify-center">
                                                        ...
                                                        <button data-modal-hide="approve-modal-{{ $stuff->id }}"
                                                                type="button"
                            ...
                            @elseif ($stuff->is_approved)
                                <p class="text-green-600 font-bold">Approved</p>
                            @elseif ($stuff->is_pending)
                        ...
                        <td class="px-6 py-4">
                            @if ($stuff->is_approved)
                                <button data-modal-target="note-modal-{{ $stuff->id }}"
                                        data-modal-toggle="note-modal-{{ $stuff->id }}"
                            ...
                            @else
                                <svg data-tooltip-target="forbidden-note-{{ $stuff->id }}"
                                    ...
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div
                                            ...
                                            <button type="button"
                                                    ...
                                                    data-modal-hide="note-modal-{{ $stuff->id }}">
                                                <svg class="w-3 h-3" aria-hidden="true"
                                                     xmlns="http://www.w3.org/2000/svg"
                                                     fill="none" viewBox="0 0 14 14">
                                                    <path stroke="currentColor" stroke-linecap="round"
                                        ...
                                        <div class="p-4 md:p-5">
                                            <form class="space-y-4" action="{{ route('stuffs.note', $stuff->id) }}"
                                                  method="POST">
                                                @method('PATCH')
                                                @csrf
                                                <div>
                                                <textarea id="note" rows="4" name="note"
                                                          ...
                                                          required
                                                          placeholder="Tulis keterangan di sini...">{{ $stuff->note }}</textarea>
                                                </div>
                                                <button type="submit"
                        ...
                        <td class="px-6 py-4">
                            <button data-popover-target="action-{{ $stuff->id }}" data-popover-placement="left"
                                    type="button"
                            ...
                            <div data-popover id="action-{{ $stuff->id }}" role="tooltip"
                                ...
                                <ul class="text-left text-gray-500 dark:text-gray-400">
                                    <li class="hover:bg-[rgba(0,0,0,.2)]">
                                        <a href="{{ route('stuffs.show', $stuff->id) }}"
                                    ...
                                    <li class="hover:bg-[rgba(0,0,0,.2)]">
                                        <a href="{{ route('stuff-attachments.index', $stuff->id) }}"
                                           class="p-3 flex gap-3 items-center">
                                            <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    @can('printPdf', $stuff)
                                        <li class="hover:bg-[rgba(0,0,0,.2)]">
                                            <a href="{{ route('stuffs.pdf', $stuff->id) }}" target="_blank"
                                               type="button"
                                               class="p-3 flex gap-3 items-center">
                                    ...
                                    @can('update', $stuff)
                                        <li class="hover:bg-[rgba(0,0,0,.2)]">
                                            <a href="{{ route('stuffs.edit', $stuff->id) }}" type="button"
                                               class="p-3 flex gap-3 items-center">
                                                <svg class="w-4 h-4" aria-hidden="true"
                                                     xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <li class="hover:bg-[rgba(0,0,0,.2)]">
                                            <form action="{{ route('stuffs.destroy', $stuff->id) }}" method="POST"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                        class="p-3 flex gap-3 items-center w-full text-red-600">
                                                    <svg class="w-4 h-4" aria-hidden="true"
                                                         xmlns="http://www.w3.org/2000/svg"
        ...
        @if ($stuffs->hasPages())
            <nav class="flex items-center justify-between mt-4">
                <div class="text-gray-500">
                    Showing {{ $stuffs->firstItem() }} to {{ $stuffs->lastItem() }} of {{
            $stuffs->total() }}
                    results
                </div>
                <div class="flex -space-x-1">
                    @if ($stuffs->currentPage() > 1)
                        <a href="{{ $stuffs->previousPageUrl() }}" class="px-4 py-2 rounded-md hover:bg-gray-100">Previous</a>
                    @endif
                    @php
                        $totalPages = $stuffs->lastPage() >= 10 ? 10 : $stuffs->lastPage();
                    @endphp
                    @for ($i = 1; $i <= $totalPages; $i++)
                        @if ($i==$stuffs->currentPage())
                            <span class="px-4 py-2 text-gray-700 bg-gray-100 rounded-md">{{ $i }}</span>
                        @else
                            <a href="{{ $stuffs->appends(['q' => request()->query('q'), 'other' => request()->query('other')])->url($i) }}"
                               class="px-4 py-2 rounded-md hover:bg-gray-100">{{ $i }}</a>
                        @endif
                    @endfor
                    @if ($stuffs->currentPage() < $stuffs->lastPage())
                        <a href="{{ $stuffs->nextPageUrl() }}" class="px-4 py-2 rounded-md hover:bg-gray-100">Next</a>
                    @endif
                </div>
            </nav>
        @endif
