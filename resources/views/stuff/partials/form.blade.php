...
<div class="mb-5">
    <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Catatan</label>
    <textarea name="description" id="description"
        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">{{ old('description') ?? $stuff->description }}</textarea>
    @error('description')
    <small class="text-red-600">{{ $message }}</small>
    @enderror
</div>
...
<div class="flex items-center mb-4">
    <input @if ($stuff->is_user_signature_showed) checked @endif id="is_user_signature_showed"
